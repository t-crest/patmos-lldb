...
//
// Copied from ABIMacOSX_i386.cpp and altered to fit Patmos' needs.
//
//===----------------------------------------------------------------------===//

#include "ABIUnknown_Patmos.h"
...
ABIUnknown_Patmos::GetRegisterInfoArray (uint32_t &count)
{
...
size_t
ABIUnknown_Patmos::GetRedZoneSize () const
{
    return 0;
}
...
//------------------------------------------------------------------
ABISP
ABIUnknown_Patmos::CreateInstance (const ArchSpec &arch)
{
    static ABISP g_abi_sp;
    if (arch.GetTriple().getArch() == llvm::Triple::patmos)
    {
        if (!g_abi_sp)
            g_abi_sp.reset (new ABIUnknown_Patmos);
        return g_abi_sp;
    }
    return ABISP();
}
...
bool
ABIUnknown_Patmos::PrepareTrivialCall (Thread &thread, 
                                    addr_t sp, 
                                    addr_t func_addr, 
                                    addr_t return_addr, 
                                    addr_t *arg1_ptr,
...
bool
ABIUnknown_Patmos::PrepareNormalCall (Thread &thread,
                                   addr_t sp,
                                   addr_t func_addr,
                                   addr_t return_addr,
...
bool
ABIUnknown_Patmos::GetArgumentValues (Thread &thread,
...
Error
ABIUnknown_Patmos::SetReturnValueObject(lldb::StackFrameSP &frame_sp, lldb::ValueObjectSP &new_value_sp)
{
    Error error;
    if (!new_value_sp)
    {
...
ValueObjectSP
ABIUnknown_Patmos::GetReturnValueObjectImpl (Thread &thread,
...
bool
ABIUnknown_Patmos::CreateFunctionEntryUnwindPlan (UnwindPlan &unwind_plan)
{
...
bool
ABIUnknown_Patmos::CreateDefaultUnwindPlan (UnwindPlan &unwind_plan)
{
...
bool
ABIUnknown_Patmos::RegisterIsVolatile (const RegisterInfo *reg_info)
{
...
bool
ABIUnknown_Patmos::RegisterIsCalleeSaved (const RegisterInfo *reg_info)
{
    if (reg_info)
    {
...
ABIUnknown_Patmos::Initialize()
{
    PluginManager::RegisterPlugin (GetPluginNameStatic(),
                                   "ABI for bare metal patmos targets",
                                   CreateInstance);    
}
...
ABIUnknown_Patmos::GetPluginNameStatic ()
{
    static ConstString g_short_name("abi.Unknown-Patmos");
    return g_short_name;
...
//------------------------------------------------------------------
lldb_private::ConstString
ABIUnknown_Patmos::GetPluginName()
{
    return GetPluginNameStatic();
}

uint32_t
ABIUnknown_Patmos::GetPluginVersion()
{
    return 1;
}
